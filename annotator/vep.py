...
def get_chunked_annotations(variant_lines, chunk_size = 200, by_gene = False):
    """
    Runs the get_variant_annotations() function on sublists of variant_lines, chunk_size elements at a time.
    ...
    ret = [get_variant_annotations(variant_lines[sublist:sublist+chunk_size], by_gene = by_gene) for sublist in range(0, len(variant_lines), chunk_size)]
...
def merge_variant_annotation(variant, annotation, consequences_list) -> list:
    """
    Parse a single variant and its associated annotations, extracting annotations according to a list of possible consequences keys.
    This function is intended to be used ie. by looping on a zip of the full list of variants/annotations.

    Args:
        variant (list):          a list corresponding to a single variant from the list returned from vcf.parse_vcf()
        annotation (dict):       a dict of VEP annotation, corresponding to get_variant_annotations([variant])
        consequence_list (list): a list of strings corresponding to the consequences dicts in VEP annotations. 
    
    Returns:
        a list of variant annotations for the provided variant. One element per annotated gene/transcript. 
    """
    ...
    ## If we have transcript_consequences or intergenic_consequences, loop through and extract gene_id, gene_symbol, transcript_id, impact, and consequence_terms
    if features:
        ## Flatten the list
    ...
    ## colocated_variants is a list of dicts, as multiple variant databases are used (HGMD, COSMIC, dbSNP)
    ...
    ## If there's annotated variants at this position (colocated variants), pull COSMIC IDs and minor allele frequencies
    if colocated_data:
        ## Check for COSMIC IDs. VEP doesn't match COSMIC ID with the actual allele of interest, so in this case we return the list of all COSMIC variants at this locus.
        cosmic_strings = [element.get("id") for element in colocated_data if element.get("allele_string") == "COSMIC_MUTATION"]
        if cosmic_strings:
        ...
        ## Need to do two loops here, first to get all the "id" values, second to select those starting with "rs"
        ...
        ## Extract rs IDs
        if rs_strings: ## Return the first dbSNP ID. 
            if len(rs_strings) > 1:
                raise ValueError(rs_strings)
            rs_string = rs_strings[0]
        else:
            rs_string = "NA"

        ## Check for allele frequency data. Frequency is a dict, each key is an alt allele, which is another dict of allele frequencies from various sources, such as gnomAD
        ...
        if af_data:
            ...
            ## Get the allele frequency. We first try the "af" key, and if it isn't found, we fall back to "gnomade", finally returning "NA" if neither
    ...
    return([variant + [var_af, variant_class] + e + [rs_string, cosmic_strings, af] for e in extracted])

def annotate_variants(vcf_lines, chunk_size = 200, by_gene = False) -> list:
    """
    Annotates variants and outputs a list of lists containing variant information, variant annotation, and variant impact, one list per genetic feature that each variant impacts.

    Args:
        vcf_lines (list): a list of variants, as output by vcf.parse_vcf(); [CHROM, POS, REF, ALT, FILTER, N_VARIANT_READS, TOTAL_READS]
        chunk_size (int): number of elements in each batch API request
    
    Returns:
        A list of lists, each in this format, with "NA" for missing values: 
        [CHROM, POS, REF, ALT, FILTER, N_VARIANT_READS, TOTAL_READS, VARIANT_READ_FRACTION, VARIANT_TYPE, GENE_ID, GENE_SYMBOL, TRANSCRIPT_ID, IMPACT, CONSEQUENCE_TERMS, HGVSP, DBSNP_ID, COSMIC_ID, POP_AF"
    """
    ...
    ## Use a list comprehension to run merge_variant_annotation on every variant and its associated VEP annotations
    out_list = [annotator.vep.merge_variant_annotation(variant, annotation, consequences_list) for variant, annotation in zip(vcf_lines, annotations)]
